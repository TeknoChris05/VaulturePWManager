...
class SettingsApp(customtkinter.CTkToplevel):
    def __init__(self, account_id, update_callback=None,
    ...
    def _switch_frame(self, frame_class):
        ...
        if frame_class == AccountFrame:
            self.current_frame = frame_class(self, self.parent, self.account_id)
        else:
            self.current_frame = frame_class(self)
...
class IntroFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        label = customtkinter.CTkLabel(self, text=contact_text, text_color=master.text_color, justify="left",
                                       font=("Segoe UI", 16), wraplength=600)
        label.pack(pady=20, padx=20)


class AccountFrame(customtkinter.CTkFrame):
    def __init__(self, master, parent, account_id):
        super().__init__(master)
        self.parent = parent
        self.account_id = account_id
    ...
    def erase_account(self, confirm_window):
        try:
            ...
            mycursor.execute("DELETE FROM Account_Data_Password WHERE AccountID = %s", (self.account_id,))
            mycursor.execute("DELETE FROM Banking_Card WHERE AccountID = %s", (self.account_id,))
            mycursor.execute("DELETE FROM Network_Data WHERE AccountID = %s", (self.account_id,))
            mycursor.execute("DELETE FROM Notes_Data WHERE AccountID = %s", (self.account_id,))
            mycursor.execute("DELETE FROM Archive_Data_Password WHERE AccountID = %s", (self.account_id,))
            mycursor.execute("DELETE FROM Archive_Banking_Card WHERE AccountID = %s", (self.account_id,))
            mycursor.execute("DELETE FROM Archive_Network_Data WHERE AccountID = %s", (self.account_id,))
            mycursor.execute("DELETE FROM Archive_Notes_Data WHERE AccountID = %s", (self.account_id,))
            login_database.commit()

            # Now, delete the account (parent row).
            mycursor.execute("DELETE FROM Account WHERE AccountID = %s", (self.account_id,))
            login_database.commit()
...
class ThemesFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        self.MainF_button = customtkinter.CTkButton(self, text="Change Main_Frame Color", border_width=2,
                                                    border_color="#7289DA", fg_color=master.theme_color,
                                                    hover_color="#d4af37", text_color=master.text_color,
                                                    command=self.pick_mainframe_color)
            ...
            try:
                ...
                if self.master.current_frame:
                    for widget in self.master.current_frame.winfo_children():
                        if isinstance(widget, (customtkinter.CTkButton, customtkinter.CTkLabel, customtkinter.CTkEntry,
                                               customtkinter.CTkSwitch, customtkinter.CTkTextbox,
...
class HelpFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        help_message_label = customtkinter.CTkLabel(help_message_frame, text=help_message, text_color=master.text_color,
        ...
        for question, answer in contact_text:
            ...
            question_label = customtkinter.CTkLabel(text_frame, text=f"• {question}", text_color=master.text_color,
            ...
            answer_label = customtkinter.CTkLabel(text_frame, text=answer, text_color=master.text_color,
                                                  font=("Segoe UI", 16), fg_color=master.theme_color, anchor="w")
...
class ContactFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        for email in email_list:
            email_label = customtkinter.CTkLabel(email_frame, text=email, text_color=master.text_color,
                                                 font=("Segoe UI", 14), fg_color=master.theme_color, height=40)
...
def opening_settings(account_id, update_callback=None, parent=None):
    settings_app = SettingsApp(account_id, update_callback=update_callback, parent=parent)
